...
void handle_connections(int sock, int port) {
  ...
  // Yes, in a typical server, an infinite loop is normal and expected because the server should continuously listen for and accept new client connections. But while (True) is not a good way to implement it, we can use a signal handler to gracefully shut down the server when needed.
  while (true) {
...
int main() {
  ...
  // A better name would be bind_and_listen since that's exactly what it does (two distinct syscall-level operations).
